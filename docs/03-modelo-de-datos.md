...
- `TRANSACTION.duplicate_of` referencia otra `TRANSACTION` del mismo usuario cuando ambas describen probablemente el mismo gasto real. El criterio de detección, y cuándo se puebla la columna, están en [reglas de dominio § 7](reglas-de-dominio.md#7-chequeo-de-duplicados-entre-origen-manual-y-automático).
