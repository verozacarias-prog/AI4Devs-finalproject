...
- En `PENDING_TRANSACTION`, `expires_at` es nulo si y solo si `recurring_expense_id` está informado (`CHECK`): los pendientes de un recurrente no vencen y todos los demás sí. `recurring_expense_id` y `occurrence_date` van los dos nulos o los dos informados (`CHECK`), con `UNIQUE (recurring_expense_id, occurrence_date)`: una regla recurrente genera como mucho un pendiente por fecha, igual que como mucho un movimiento.
