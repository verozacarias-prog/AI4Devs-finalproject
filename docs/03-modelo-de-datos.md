...
erDiagram
    ...
    TRANSACTION {
        ...
        uuid recurring_rule_id FK "NULLABLE, set when generated by a recurring rule"
        uuid card_purchase_id FK "NULLABLE, set when it is an installment of a credit card purchase"
...
Las cuentas y lo que genera movimientos sobre ellas: compras en cuotas, resúmenes de tarjeta,
transferencias y reglas recurrentes.
...
erDiagram
    ...
    CARD_STATEMENT {
        ...
        date due_date "NOT NULL, CHECK (due_date > closing_date)"
...
Una compra con tarjeta de crédito. No es un movimiento: es la regla que genera uno por cuota, cada uno imputado al presupuesto del mes en que vence ([reglas de dominio § 13](reglas-de-dominio.md#13-tarjetas-de-crédito-y-transferencias)). Guarda el dueño del presupuesto, confirmado una sola vez al comprar, y no un período, por el mismo motivo que `RECURRING_RULE`: sus cuotas caen en períodos que todavía no existen. Lo comprometido a futuro de una tarjeta son las cuotas de sus compras que todavía no se generaron. El fundamento está en el [ADR 0012](adr/0012-tarjetas-de-credito-y-transferencias.md).
...
Ver también, en otra tabla: [el presupuesto individual propio, en RECURRING_RULE](#recurring_rule) · [la marca de edición y el borrado lógico, en TRANSACTION](#transaction).

##### CARD_STATEMENT

Cada resumen de una tarjeta. Sus fechas se generan a partir de `closing_day` y `due_day` de la cuenta, y el usuario puede corregirlas mientras el resumen está abierto. Al cerrarse, el proceso programado genera las cuotas que entran en él.
...
Ver también, en otra tabla: [las cuotas de tarjeta, en TRANSACTION](#transaction).
