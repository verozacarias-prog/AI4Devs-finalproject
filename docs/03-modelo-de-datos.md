...
- `TRANSACTION.duplicate_of` referencia otra `TRANSACTION` del mismo usuario cuando ambas describen probablemente el mismo gasto real. La columna se crea desde la migración inicial, pero la lógica que la puebla depende de la carga por email, que es could-have (ver [1.2](01-producto.md#12-características-y-funcionalidades-principales)). El criterio de detección está en [reglas de dominio § 7](reglas-de-dominio.md#7-chequeo-de-duplicados-entre-origen-manual-y-automático).
