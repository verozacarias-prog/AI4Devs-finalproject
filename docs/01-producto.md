...
- **Transferencias entre cuentas propias**: sacar efectivo, pasar plata de un banco a una billetera o comprar dólares mueve saldos sin contar como gasto ni ingreso. Es parte del saldo calculado: sin transferencias, esos movimientos dejarían mal los saldos. Detalle en [reglas de dominio § 13](reglas-de-dominio.md#13-tarjetas-de-crédito-y-transferencias).
...
- **Alta por WhatsApp**: la persona se da de alta conversando, con una parte obligatoria y corta (consentimiento, datos básicos, permiso para avisos y sus cuentas) y una configuración opcional de categorías y perfil financiero para mejorar los consejos. Detalle en [reglas de dominio § 11](reglas-de-dominio.md#11-alta-de-usuario-consentimiento-y-mensajes-proactivos) y [HU6](05-historias-de-usuario.md).
...
- **Trazabilidad de origen**: cada movimiento guarda si se cargó manual (WhatsApp) o automático (movimiento recurrente, o email cuando se implemente), lo que habilita auditoría y una métrica de cuánto está resolviendo cada vía automática.
...
- **Tarjetas de crédito**: la tarjeta es una cuenta con cierre y vencimiento; cada compra, en uno o varios pagos, pesa en el presupuesto del mes en que vence cada cuota, pagar el resumen es una transferencia, y cada resumen se concilia contra el total del banco para capturar intereses, impuestos y devoluciones. El fundamento está en el [ADR 0012](adr/0012-tarjetas-de-credito-y-transferencias.md).
- **Movimientos recurrentes**: el usuario configura un gasto o un ingreso que se repite (alquiler, suscripciones, servicios fijos, el sueldo) indicando periodicidad, categoría, cuenta de la que sale o a la que entra, y presupuesto al que se imputa. Con eso definido una sola vez, el sistema genera el movimiento completo en cada ciclo, sin volver a cargarlo ni volver a preguntar nada.
...
- **Soporte multimoneda**: un presupuesto recibe movimientos en distintas monedas (ej. ARS y USD) y los convierte a la moneda primaria usando una cotización de referencia. La cotización se busca sola desde fuentes configurables, empezando por Argentina; en países sin fuente, el asistente pregunta el valor. El mecanismo está en el [ADR 0011](adr/0011-cotizaciones-con-adaptador-generico-configurable.md).
